...
layout(set = 0, binding = 0, rgba32f) readonly uniform image2D IN_COLOR;
...
uint colorToBin(vec3 hdrColor, float minLogLum, float inverseLogLumRange) {
  float lum = dot(hdrColor, RGB_TO_LUM); // HDR->Luminance
  if (lum < EPSILON) {
    return 0;
  }
...
shared uint HistogramShared[256];  // 256个位置为横坐标，统计不同Luminance的像素数量
