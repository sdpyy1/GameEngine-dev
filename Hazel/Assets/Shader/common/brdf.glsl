...
void Init( inout BxDFContext Context, vec3 N, vec3 V, vec3 L )
{
    ...
    Context.NoL = max(0.0, Context.NoL);

    // 不知道为什么UE的这种实现方式，让高光特别不自然
    // float InvLenH = rsqrt( 2 + 2 * Context.VoL );
    // Context.NoH = saturate(( Context.NoL + Context.NoV ) * InvLenH);
	// Context.VoH = saturate(InvLenH + InvLenH * Context.VoL);

    vec3 hv = normalize(L + V);
	Context.NoH = max(0.0, dot(N, hv));
    Context.VoH = max(0.0, dot(hv, V));
}

////////////////////////////////////////// Form Engine\Shaders\Private\BRDF.ush//////////////////////////////////////////////////////////
...
////////////////////////////////////////// G = Shadowing and masking | Vis = G / (4*NoL*NoV)//////////////////////////////////////////////////////////
float Vis_Implicit()
{
	return 0.25;
}
