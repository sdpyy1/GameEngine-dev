...
layout(set = 1, binding = 4, rgba32f) uniform image2D DIRECT_COLOR;
layout(set = 1, binding = 5, rgba32f) uniform image2D IN_DIRECT_COLOR;
...
void main() 
{
	...
	vec3 outDirectColor = vec3(0.0f); // 存储直接光 为了SVGF
	vec3 outInDirectColor = vec3(0.0f); // 存储间接光 为了SVGF
	vec3 SVGFNeedAlbedo = vec3(0.0f);
	...
	for(int i = 0; i < SETTING.numSamples; i++){
		...
		for(uint b = 0; b <= SETTING.numBounce; b++){
			...
			/////////////////////////////////////////////// 存储直接光，用于SVGF ///////////////////////////////////////////////
			if(SETTING.indirectOnly == 0 && b == 0 && SETTING.historyActive == 0 && SETTING.numSamples == 1){  // 不做时域积累 SPP=1 时才启动
				outDirectColor = outColor;
				SVGFNeedAlbedo= albedo;
			}
