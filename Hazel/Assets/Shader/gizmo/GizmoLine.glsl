...
{
	gl_Position = CAMERAINFO.data.projNoJetter * CAMERAINFO.data.view * vec4(from, 1.0f);
	OUT_COLOR = color;
	EmitVertex();

	gl_Position = CAMERAINFO.data.projNoJetter * CAMERAINFO.data.view * vec4(to, 1.0f);
	OUT_COLOR = color;
	EmitVertex();
